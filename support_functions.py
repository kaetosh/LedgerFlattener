...
def print_start_text() -> None:
    ...
    sections = [
        (
            ...
            [
                "1) Особенности:",
                "   - используйте только формат .xlsx, устаревшие .xls и прочие форматы Excel не будут обработаны;",
                "   - список необработанных в результате ошибок регистров отобразится в конце работы скрипта;",
                "   - пакетный режим, получите единую таблицу из множества регистров;",
                "   - допускается обработка регистров, выгруженных с опциями Количество и Валюта;",
                "   - большие файлы обрабатываются дольше (на каждые 500 тыс. строк ~ 2 мин.);",
                "   - результаты обработки не заменяют экспертную проверку, обязательно выборочно сверяйтесь с исходными данными;",
                "   - возможны неучтенные ошибки в работе программы, будут устраняться по мере обнаружения.\n",
                
                "2) Инструкция:",
                "   - выберете вид регистра для обработки;",
                "   - ознакомтесь с требованиями к регистру;",
                "   - перетащите в окно программы файл регистра или папку с регистрами;",
                "   - дождитесь окончания обработки, готовый файл откроется автоматически.\n",
            ]
        ),
    ]
...
def print_instruction_color(type_registr: Literal['posting', 'card', 'analisys', 'turnover', 'generalosv']) -> None:
    ...
    REGISTER_HEADERS_MAP = {
        ...
        'accountosv': ['Субконто|Сальдо на начало периода|Оборот за период|Кред. оборот|Сальдо на конец периода',
                     'Счет|Начальное сальдо Дт|Начальное сальдо Кт|Оборот Дт|Оборот Кт|Конечное сальдо Дт|Конечное сальдо Кт'],
        'generalosv': ['Счет|Наименование|Сальдо на начало периода|Оборот за период|Сальдо на конец периода',
...
def validate_paths(paths: Iterable[Path]) -> List[Path]:
    ...
    if invalid_paths:
        error_message = Fore.RED + 'Неверные пути к папке или файлу/файлам:\n' + '\n'.join(invalid_paths)
