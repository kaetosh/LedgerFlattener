...
class Turnover_UPPFileProcessor(FileProcessor):
    ...
    def process_file(self, file_path: Path) -> pd.DataFrame:
        ...
        def normalize_account_column(col_name):
            col_str = str(col_name)
            
            # Обрабатываем случаи типа "8.0", "21.0", "76.0"
            if re.match(r'^\d+\.0$', col_str):
                num = int(float(col_str))
                return f"{num:02d}" if num < 10 else str(num)
            
            # Обрабатываем случаи типа "8", "21" (без .0)
            elif col_str.isdigit():
                num = int(col_str)
                return f"{num:02d}" if num < 10 else str(num)
            
            # Оставляем как есть для текстовых значений
            return col_str
    
        # df.columns = [normalize_account_column(col) for col in df.columns]
        ...
        # Добавляем _до к столбцам между Деб. оборот и Кред. оборот
        for i in range(debit_idx + 1, credit_idx):
            new_cols[i] = f"{cols[i]}_до"
        
        # Добавляем _ко к столбцам между Кред. оборот и Кон. сальдо деб.
        for i in range(credit_idx + 1, end_debit_idx):
            new_cols[i] = f"{cols[i]}_ко"
        ...
        '''Транспонируем количественные и валютные данные
                          {'Нач. сальдо деб.': 'Дебет_начало',
                           'Нач. сальдо кред.': 'Кредит_начало',
                           'Деб. оборот': 'Дебет_оборот',
                           'Кред. оборот': 'Кредит_оборот',
                           'Кон. сальдо деб.': 'Дебет_конец',
                           'Кон. сальдо кред.': 'Кредит_конец'}
        '''
